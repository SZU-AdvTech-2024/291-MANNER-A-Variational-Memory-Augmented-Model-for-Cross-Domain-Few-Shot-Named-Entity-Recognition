    ...
    def get_similar_memory(self, hiddens):
        ...
        for typ in range(len(self.types_list)):
            if typ in self.memory and self.memory_size[typ] >= self.min_centroids:
                ...
                # perform standard barycentric mapping   计算最优传输计划T*
                transp = coupling / np.sum(coupling, axis=1)[:, None]
                # set nans to 0
                transp[~np.isfinite(transp)] = 0
                # compute transported samples   公式5
...
class ProtoBertForTokenClassification(BertForTokenClassification):
    ...
    def loss(
        ...
        # split suppport set representation and query set representation    从获得的表示中分离出支持集和查询集的
        ...
        for typ in range(1, len(types_list)+1):
            ...
            # 公式4 从memory中找出与支持集表示最相似的类k*，选择其中top-k个最相似的记忆向量，同时保证没有None的
        ...
        # obtain prior distribution of prototypes  公式7 计算原型的先验分布，支持集和Memory的
        prototype_mu_prior = torch.cat(mu_prior_list, dim=0)
        prototype_logstd_prior = torch.cat(logstd_prior_list, dim=0)

        # infer posterior distribution of prototypes  Learning in Source Domain中的操作 变分分布近似原型的后验
...
#
            ...
            orth_loss = torch.norm(torch.mm(mu_prior.T, mu_prior) - torch.eye(mu_prior.size(1)).to(mu_prior.device))
            loss += lamda * orth_loss
...
#       corpus是验证集
        ...
        #初始化转移矩阵，O类不能直接转移到其他任何类型，B和I不能直接转移到O，S和E不能直接转移到B和I
